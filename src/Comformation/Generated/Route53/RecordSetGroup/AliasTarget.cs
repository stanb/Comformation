...
namespace Comformation.Route53.RecordSetGroup
{
    ...
    public class AliasTarget
    {
        ...
        /// APIs 				 					 Specify the applicable domain name for your API. You can get the applicable value
        /// using the AWS CLI command 						get-domain-names: 					 						 						 					 							 For regional
        /// APIs, specify the value of regionalDomainName. 						 							 For edge-optimized APIs, specify the
        /// value of distributionDomainName. This is the name of the 								associated CloudFront distribution,
        /// such as da1b2c3d4e5. cloudfront. net. 						 					 Note The name of the record that you&#39;re creating
        /// must match a custom domain name for your API, such as 							api. example. com. 				 			 				 Amazon
        /// Virtual Private Cloud interface VPC endpoint 				 					 Enter the API endpoint for the interface
        /// endpoint, such as 					vpce-123456789abcdef01-example-us-east-1a. elasticloadbalancing. us-east-1.
        /// vpce. amazonaws. com. For edge-optimized APIs, 					this is the domain name for the corresponding
        /// CloudFront distribution. You can get the value of DnsName using the AWS CLI command
        /// 					describe-vpc-endpoints. 				 			 				 CloudFront distribution 				 					 Specify the domain
        /// name that CloudFront assigned when you created your distribution. 					 Your CloudFront distribution
        /// must include an alternate domain name that matches the name of the record. 						For example, if the
        /// name of the record is acme. example. com, your CloudFront distribution must 						include acme.
        /// example. com as one of the alternate domain names. For more information, see 						Using Alternate
        /// Domain Names (CNAMEs) in the Amazon CloudFront Developer Guide. 					 You can&#39;t create a record in a
        /// private hosted zone to route traffic to a CloudFront distribution. 					 Note For failover alias
        /// records, you can&#39;t specify a CloudFront distribution for both the primary and secondary records.
        /// 						A distribution must include an alternate domain name that matches the name of the record.
        /// However, the primary and secondary records 						have the same name, and you can&#39;t include the same
        /// alternate domain name in more than one distribution. 				 			 				 Elastic Beanstalk environment
        ...
        /// environment. For example, the domain name 						my-environment. us-west-2. elasticbeanstalk. com is
        /// a regionalized domain name. 					 Important For environments that were created before early 2016,
        /// the domain name doesn&#39;t include the region. To route traffic 							to these environments, you must
        /// create a CNAME record instead of an alias record. Note that you can&#39;t create a 							CNAME record
        /// for the root domain name. For example, if your domain name is example. com, you can create a record
        ...
        /// environment. 					 For Elastic Beanstalk environments that have regionalized subdomains, specify the
        /// CNAME attribute for the environment. 						You can use the following methods to get the value of the
        /// CNAME attribute: 					 						 						 						 					 							 AWS Management Console: For information
        /// about how to get the value by using the console, 								see Using Custom Domains with AWS Elastic
        /// Beanstalk in the 								AWS Elastic Beanstalk Developer Guide. 						 							 Elastic Beanstalk
        ...
        /// 		
        /// Applies only to alias records with any routing policy: When EvaluateTargetHealth is 			true, an
        /// alias record inherits the health of the referenced AWS resource, such as an ELB load balancer or
        /// 			another record in the hosted zone.
        /// 		
        /// Note the following:
        /// 		
        /// 			 			 			 			 			 		 				 CloudFront distributions 				 					 You can&#39;t set EvaluateTargetHealth
        /// to true when the alias target is a CloudFront distribution. 				 			 				 Elastic Beanstalk
        /// environments that have regionalized subdomains 				 If you specify an Elastic Beanstalk environment
        /// in DNSName and the environment contains an ELB load balancer, 					Elastic Load Balancing routes
        /// queries only to the healthy Amazon EC2 instances that are registered with the load balancer. (An
        /// environment automatically 					contains an ELB load balancer if it includes more than one Amazon EC2
        /// instance. ) If you set EvaluateTargetHealth to 					true and either no Amazon EC2 instances are
        /// healthy or the load balancer itself is unhealthy, 					Route 53 routes queries to other available
        /// resources that are healthy, if any. 					 If the environment contains a single Amazon EC2 instance,
        /// there are no special requirements. 				 			 				 ELB load balancers 				 Health checking behavior
        /// depends on the type of load balancer: 					 						 						 					 Classic Load Balancers: If you
        /// specify an ELB Classic Load Balancer in 							DNSName, Elastic Load Balancing routes queries only
        /// to the healthy Amazon EC2 instances that are registered with the load balancer. 							If you set
        /// EvaluateTargetHealth to true and either no EC2 instances are healthy or the 							load balancer
        /// itself is unhealthy, Route 53 routes queries to other resources. 						 Application and Network Load
        /// Balancers: If you specify an ELB 							Application or Network Load Balancer and you set
        /// EvaluateTargetHealth to true, 							Route 53 routes queries to the load balancer based on the
        ...
        /// considered unhealthy. 						 					 Note When you create a load balancer, you configure settings for
        /// Elastic Load Balancing health checks; they&#39;re not Route 53 health checks, but 							they perform a
        /// similar function. Do not create Route 53 health checks for the EC2 instances that you register with
        /// an ELB load balancer. 				 			 				 S3 buckets 				 There are no special requirements for setting
        /// EvaluateTargetHealth to true 					when the alias target is an S3 bucket. 				 			 				 Other records
        /// in the same hosted zone 				 If the AWS resource that you specify in DNSName is a record or a group
        /// of records 					(for example, a group of weighted records) but is not another alias record, we
        /// recommend that you associate a health check 					with all of the records in the alias target. For
        /// more information, see 					What Happens 						When You Omit Health Checks? in the Amazon Route 53
        /// Developer Guide. 				 			
        /// 		 		
        ...
        /// 		
        /// 			 			 			 			 			 			 			 			 		 				 Amazon API Gateway custom regional APIs and edge-optimized
        /// APIs 				 					 Specify the hosted zone ID for your API. You can get the applicable value using the
        /// AWS CLI command 					get-domain-names: 					 						 						 					 For regional APIs, specify the
        /// value of regionalHostedZoneId. For edge-optimized APIs, specify the value of
