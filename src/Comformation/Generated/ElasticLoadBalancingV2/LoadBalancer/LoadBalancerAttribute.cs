...
namespace Comformation.ElasticLoadBalancingV2.LoadBalancer
{
    ...
    public class LoadBalancerAttribute
    {
        ...
        /// http. x_amzn_tls_version_and_cipher_suite. enabled - Indicates whether the two headers
        ...
        /// false. The default is false. routing. http. xff_client_port. enabled - Indicates whether the
        /// X-Forwarded-For header should preserve the source port that the client used to connect to the load
        /// balancer. The possible values are true and false. The default is false. routing. http2. enabled -
        /// Indicates whether HTTP/2 is enabled. The possible values are true and false. The default is true.
        /// Elastic Load Balancing requires that message header names contain only alphanumeric characters and
        /// hyphens. waf. fail_open. enabled - Indicates whether to allow a WAF-enabled load balancer to route
        /// requests to targets if it is unable to forward the request to AWS WAF. The possible values are true
