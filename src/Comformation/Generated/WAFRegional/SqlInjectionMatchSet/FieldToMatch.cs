...
namespace Comformation.WAFRegional.SqlInjectionMatchSet
{
    ...
    /// Specifies the part of a web request that you want AWS WAF to search, such as a specific header or a query
    /// string.
    /// https://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/aws-properties-wafregional-sqlinjectionmatchset-fieldtomatch.html
    /// </summary>
    public class FieldToMatch
    {
        ...
        /// the HTTP request body, such as data from a form. The request body immediately follows the request
        /// headers. Note that only the first 8192 bytes of the request body are forwarded to AWS WAF for
        /// inspection. To allow or block requests based on the length of the body, you can create a size
        /// constraint set. SINGLE_QUERY_ARG: The parameter in the query string that you will inspect, such as
        /// UserName or SalesRegion. The maximum length for SINGLE_QUERY_ARG is 30 characters. ALL_QUERY_ARGS:
        /// Similar to SINGLE_QUERY_ARG, but rather than inspecting a single parameter, AWS WAF will inspect all
        /// parameters within the query for the value or regex pattern that you specify in TargetString.
        /// 			 	
        /// Required: Yes
        /// Type: String
